...
impl Lexer {
    ...
    pub fn exec(&mut self) -> Vec<Token> {
        ...
        while let Some(head_line) = self.line.clone() {
            ...
            // Heading
            else if Regex::new(r"^(#{1,6})\s")
                .unwrap()
                .is_match(head_line.as_str())
            {
                let depth: i8 = Regex::new("#")
                    .unwrap()
                    .find_iter(head_line.as_str())
                    .count() as i8;
                let text: String = Regex::new("#")
                    .unwrap()
                    .replace_all(head_line.as_str(), "")
                    .trim()
                    .to_string();
            ...
            // Code
            else if Regex::new(r"^\B[(`{3})(\t{1})]")
                .unwrap()
                .is_match(head_line.as_str())
            {
                let mut find_lang: Option<_> = Regex::new(r"[\w[0-9]]+")
                    .unwrap()
                    .captures(head_line.as_str());
            ...
            // List
            else if Regex::new(r"^[-+*]\s+[\W[[:punct:]]\w]")
                .unwrap()
                .is_match(head_line.as_str())
            {
            ...
            // OrderedList
            else if Regex::new(r"^([0-9]+.)\s+[\W[[:punct:]]\w]")
                .unwrap()
                .is_match(head_line.as_str())
            {
                            ...
                            .replace(
                                Regex::new(r"\n$")
                                    ...
                                    .replace(
                                        Regex::new(r"^>+")
                                            .unwrap()
                                            .replace(self.line.clone().unwrap().as_str(), "")
                                            .to_string()
                                            .as_str(),
                                        "",
                                    )
    ...
    fn code_token(&mut self, lang: String) -> Token {
        ...
        while is_code {
            if Regex::new(r"^\B[(`{3})(\t{1})]")
                .unwrap()
                .is_match(self.line.clone().unwrap_or("".to_string()).as_str())
            {
                is_code = false;
            } else {
